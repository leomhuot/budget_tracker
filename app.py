    ...
    try:
        with open(USERS_FILE, 'r', newline='') as f:
            ...
            for row in reader:
                    ...
                    return User(id=row[0], username=row[1], email=email, password_hash=row[3], role=row[4], totp_secret=totp_secret)
    ...
    try:
        with open(USERS_FILE, 'r', newline='') as f:
            ...
            for row in reader:
                email = row[2] if len(row) > 2 else None
                totp_secret = row[5] if len(row) > 5 else None
                users.append(User(id=row[0], username=row[1], email=email, password_hash=row[3], role=row[4], totp_secret=totp_secret))
    except (IOError, IndexError):
        return []
    return users
...
def update_user_totp_secret(user_id, totp_secret):
    ...
    with open(USERS_FILE, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'username', 'email', 'password', 'role', 'totp_secret'])
        for user in users:
            if user.id == user_id:
                user.totp_secret = totp_secret
                updated = True
            writer.writerow([user.id, user.username, user.email, user.password_hash, user.role, user.totp_secret])
    return updated
...
@app.route('/admin/users/demote/<user_id>')
@login_required
@admin_required
def demote_user(user_id):
    ...
    with open(USERS_FILE, 'w', newline='') as f:
        ...
        writer.writerow(['id', 'username', 'email', 'password', 'role', 'totp_secret'])
        for user in users:
            # Added condition to demote user if the ID matches
            if user.id == user_id:
                user.role = 'user'
            writer.writerow([user.id, user.username, user.email, user.password_hash, user.role, user.totp_secret])
...
def change_password():
    if request.method == 'POST':
        ...
        # Update the user's password in users.csv
        users = get_all_users()
        with open(USERS_FILE, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'username', 'email', 'password', 'role', 'totp_secret'])
        ...
        return redirect(url_for('settings')) # Redirect to settings or index after successful change
    ...
    if request.method == 'POST':
        ...
        if not user:
            ...
            for u in get_all_users():
                if u.email and u.email.lower() == username_or_email.lower():
                    user = u
                    break
...
def register():
    ...
    if request.method == 'POST':
        ...
        users = get_all_users()
        new_id = (max(int(user.id) for user in users) + 1) if users else 1
        
        # Assign role: first user is admin, others are 'user'
        role = 'admin' if new_id == 1 else 'user'
        
        # Create new user
        new_user = User(id=str(new_id), username=username, email=email, password_hash=password_hash, role=role, totp_secret=None)
